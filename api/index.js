...
const Product = mongoose.models.Product || mongoose.model('Product', new mongoose.Schema({ name:{type:String,required:true,trim:true,maxlength:120}, category:{type:String,required:true,trim:true,maxlength:80}, price:{type:Number,min:0}, imageUrl:{type:String,trim:true,maxlength:1000}, active:{type:Boolean,default:true} }, {timestamps:true}));
...
app.get('/api/products', async (_,res)=>{ try{await connect();res.json({products:await Product.find({active:true}).sort({createdAt:-1}).lean()});}catch{res.status(500).json({message:'Could not load products.'});} });
app.post('/api/products',authenticate,owner,async(req,res)=>{try{await connect();const product=await Product.create(req.body);res.status(201).json({product});}catch{res.status(400).json({message:'Product needs a name and category.'});}});
app.patch('/api/products/:id',authenticate,owner,async(req,res)=>{const product=await Product.findByIdAndUpdate(req.params.id,req.body,{new:true,runValidators:true});product?res.json({product}):res.status(404).json({message:'Product not found.'});});
